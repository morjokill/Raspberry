package ru.itis;

/**
 * Class for constant values of notes
 */
public class NoteConstants {
    public static final int cL = 129;
    public static final int cLS = 139;
    public static final int dL = 146;
    public static final int dLS = 156;
    public static final int eL = 163;
    public static final int fL = 173;
    public static final int fLS = 185;
    public static final int gL = 194;
    public static final int gLS = 207;
    public static final int aL = 219;
    public static final int aLS = 228;
    public static final int bL = 232;

    public static final int c = 261;
    public static final int cS = 277;
    public static final int d = 294;
    public static final int dS = 311;
    public static final int e = 329;
    public static final int f = 349;
    public static final int fS = 370;
    public static final int g = 391;
    public static final int gS = 415;
    public static final int a = 440;
    public static final int aS = 455;
    public static final int b = 466;

    public static final int cH = 523;
    public static final int cHS = 554;
    public static final int dH = 587;
    public static final int dHS = 622;
    public static final int eH = 659;
    public static final int fH = 698;
    public static final int fHS = 740;
    public static final int gH = 784;
    public static final int gHS = 830;
    public static final int aH = 880;
    public static final int aHS = 910;
    public static final int bH = 933;
}
